import { Renderer, Program, Mesh, Color, Triangle } from './lib/ogl.js';
...
export function createFaultyTerminal(container, options = {}) {
  ...
  const renderer = new Renderer({ dpr, alpha: true, depth: false });
  ...
  return {
    ...
    setTint: (newHex) => {
      ...
      program.uniforms.uTint.value = new Color(r, g, b);
    },
    setGlitch: (amt) => {
      program.uniforms.uGlitchAmount.value = amt;
    },
    setScanline: (intensity) => {
      program.uniforms.uScanlineIntensity.value = intensity;
    }
  };
}
